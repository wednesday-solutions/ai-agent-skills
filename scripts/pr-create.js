...
async function generatePRContent(commits, diff) {
  const apiKey = process.env.OPENROUTER_API_KEY || process.env.ANTHROPIC_API_KEY;
  if (!apiKey || !diff.trim()) return null;

  const commitList = commits.map((c, i) => `${i + 1}. ${c}`).join('\n');
  const prompt = `You are writing two sections of a GitHub Pull Request.
...
- "description": 2–4 sentences explaining what changed and why (not how). No bullet points, no headings.
- "steps": an array of 3–6 strings, each a concrete step a reviewer should follow to test this PR works correctly. Start each step with a verb (e.g. "Run", "Open", "Click", "Check"). Be specific to what actually changed.
...
function buildBody(commits, aiContent, ticket, base, isStacked) {
  ...
  const stepsToTest = aiContent?.steps?.length
    ? aiContent.steps.map((s, i) => `${i + 1}. ${s}`).join('\n')
...
async function main() {
  ...
  if (hasApiKey) {
    ...
    aiContent = await generatePRContent(commits, diff);
    if (aiContent?.description) ok('Description + steps to test generated by AI');
    else info('AI generation failed — using commit messages');
  } else {
    info('No API key — using commit messages for description');
  }

  const body = buildBody(commits, aiContent, ticket, base, isStacked);
